...
var addTask = function(date) {
    //write content
    $('#start-date-user-input').value = "date.format()";
    $("#new-task").show();
    $( "#new-task" ).dialog({
        title: "New Task",
        modal: true,
            ...
            'Create Task!': function () {
                ...
                if (invalidTime(s.options[s.selectedIndex].value, e.options[e.selectedIndex].value)) {
                    ...
                    $.ajax({
                        ...
                        data: {title: target["title"], date: target["date"], startT: target["startT"], endT: target["endT"], startD: target["startD"], endD: target["endD"], location: target["location"], info: target["info"], bg_color: target["bgC"], text_color: target["textC"]},
